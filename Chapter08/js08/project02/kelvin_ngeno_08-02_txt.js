...
Ball.prototype.moveWithin = function(container) {
	...
	let ballLaft = this.xPos;
	let ballBottom = this.yPost + this.radius;
	let ballRight = this.xPost + this.radius;
	
	if (ballTop <= container.yPos || ballBottom >= container.yPos + container.height) {
      this.yVelocity *= -1; // Reverse Y velocity
  }

  // Left or right boundary
  if (ballLeft <= container.xPos || ballRight >= container.xPos + container.width) {
      this.xVelocity *= -1; // Reverse X velocity
  }
